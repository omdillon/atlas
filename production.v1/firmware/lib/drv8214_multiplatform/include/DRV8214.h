...
// Result of a single burst read of the 7 contiguous status registers
// DRV8214_FAULT (0x00) through DRV8214_REG_STATUS3 (0x06). See
// DRV8214::getStatusBurst() -- same values getFaultStatus()/getRippleCount()/
// getMotorVoltage()/getMotorCurrent()/getDutyCycle() return individually,
// just fetched in one I2C transaction instead of up to six.
struct DRV8214_StatusBurst {
    uint8_t  fault     = 0;
    uint16_t ripple    = 0;
    float    voltage   = 0.0f;
    float    current   = 0.0f;
...
class DRV8214 {
    ...
    public:
        ...
        // Reads FAULT..REG_STATUS3 (0x00-0x06) in 4 I2C transactions instead
        // of the 6 separate transactions getFaultStatus() + getRippleCount()
        // + getMotorVoltage() + getMotorCurrent() + getDutyCycle() cost when
        // called individually -- same conversion formulas, same values, just
        // cheaper for periodic telemetry polling. NOT a single 7-byte burst:
        // bench testing showed this chip's register auto-increment does not
        // span from the FAULT/RC_STATUS block (0x00-0x03, burst-read as one
        // transaction here) into the REG_STATUS block (0x04-0x06, read
        // individually here) -- see the implementation comment in
        // DRV8214.cpp for the evidence. Currently implemented for the
        // Arduino platform; on STM32 the 4-byte burst portion works too
        // since that platform's implementation supports multi-byte reads.
        DRV8214_StatusBurst getStatusBurst();
        uint8_t  getCONFIG0();
