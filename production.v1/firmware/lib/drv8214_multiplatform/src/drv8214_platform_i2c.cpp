...
void drv8214_i2c_read_registers(uint8_t device_address, uint8_t start_reg, uint8_t* buffer, uint8_t count) {
    ...
    Wire.endTransmission(false); // Send restart condition
    uint8_t received = Wire.requestFrom(device_address, count);
    uint8_t i = 0;
    for (; i < received && i < count; i++) {
        buffer[i] = Wire.read();
    }
    for (; i < count; i++) {
        buffer[i] = 0; // Error or short read -- zero-fill the rest
    }
#elif defined(DRV8214_PLATFORM_STM32)
    if (HAL_I2C_Master_Transmit(drv_i2c_handle, (uint16_t)(device_address << 1), &start_reg, 1, HAL_MAX_DELAY) == HAL_OK &&
        HAL_I2C_Master_Receive(drv_i2c_handle, (uint16_t)(device_address << 1), buffer, count, HAL_MAX_DELAY) == HAL_OK) {
        return;
    }
    for (uint8_t i = 0; i < count; i++) buffer[i] = 0; // Error
#endif
}
