...
DRV8214_StatusBurst DRV8214::getStatusBurst() {
    // Bench-confirmed (2026-08-13): reading all 7 status registers (FAULT
    // through REG_STATUS3, 0x00-0x06) in ONE continuous burst returns correct
    // data for the first 4 bytes but saturated 0xFF for the last 3, on every
    // motor. The 0xFF (not the 0x00 this driver's own short-read path would
    // produce) proves the ESP32 believed the transaction completed -- the
    // DRV8214 itself stopped supplying real data partway through, meaning its
    // internal register auto-increment doesn't span from the RC_STATUS block
    // (0x00-0x03) into the REG_STATUS block (0x04-0x06) in one transaction.
    // Fix: burst only the block that's proven to work (FAULT+RC_STATUS1-3),
    // and read REG_STATUS1-3 (voltage/current/duty) individually, same as the
    // single-register getters below -- still 4 I2C transactions instead of 6.
    uint8_t raw[4] = {0, 0, 0, 0};
    drv8214_i2c_read_registers(address, DRV8214_FAULT, raw, 4);
    // raw[0]=FAULT(0x00) raw[1]=RC_STATUS1(0x01,unused) raw[2]=RC_STATUS2(0x02,ripple lo) raw[3]=RC_STATUS3(0x03,ripple hi)

    DRV8214_StatusBurst result;
    result.fault  = raw[0];
    result.ripple = (uint16_t)((raw[3] << 8) | raw[2]);   // same packing as getRippleCount()

    uint8_t voltageReg = drv8214_i2c_read_register(address, DRV8214_REG_STATUS1);
    uint8_t currentReg = drv8214_i2c_read_register(address, DRV8214_REG_STATUS2);
    uint8_t dutyReg     = drv8214_i2c_read_register(address, DRV8214_REG_STATUS3);

    // Same branches as getMotorVoltage() -- kept in sync with it deliberately.
    if (config.voltage_range) {
        result.voltage = (voltageReg / 255.0f) * 3.92f;
    } else if (config.ovp_enabled) {
        result.voltage = (voltageReg > 0xB0) ? 11.0f : (voltageReg / 176.0f) * 11.0f;
    } else {
        result.voltage = (voltageReg / 255.0f) * 15.7f;
    }

    // Same formula as getMotorCurrent().
    result.current = (currentReg / 192.0f) * config.MaxCurrent;

    // Same formula as getDutyCycle().
    result.dutyCycle = ((dutyReg & REG_STATUS3_IN_DUTY) * 100) / 63;
